...
# demo <group> <loads-in-app: yes|no> <filename> <title> <description> -- <ffmpeg args...>
#
# Runs ffmpeg with the given args (everything between `ffmpeg` and the output
# path, so input options like -t belong before their -i), stamps the title and
# description into the file's metadata tags (unless NO_META=1, for the
# stripped-metadata demo), and records a manifest entry.
demo() {
  local group=$1 loads=$2 file=$3 title=$4 desc=$5
  shift 5
  if [ "${1:-}" = "--" ]; then shift; fi
  echo "==> $file"
  local meta=(-metadata "title=$title" -metadata "comment=$desc")
  if [ "${NO_META:-0}" = 1 ]; then meta=(); fi
  ffmpeg -hide_banner -loglevel error -y "$@" "${meta[@]}" "$OUT/$file"
  local bytes
  bytes=$(wc -c <"$OUT/$file")
  MANIFEST_ENTRIES+=("$(printf '{"file": "%s", "group": "%s", "title": "%s", "shows": "%s", "loads_in_app": %s, "bytes": %s, "ffmpeg_args": "%s"}' \
...
demo reference yes 01-reference.mp4 \
...
demo layout yes 02-not-faststart.mp4 \
...
demo layout yes 03-fragmented.mp4 \
...
demo container yes 04-container-quicktime.mov \
...
demo container no 05-container-matroska.mkv \
...
demo container no 06-container-webm-vp9.webm \
...
demo container no 07-container-avi-mpeg4.avi \
...
demo codec yes 08-codec-h264-baseline.mp4 \
...
demo codec yes 09-codec-h264-high10.mp4 \
...
demo codec yes 10-codec-h264-high444.mp4 \
...
demo codec yes 11-codec-hevc.mp4 \
  "H.265/HEVC in MP4" \
...
demo codec yes 12-codec-av1.mp4 \
  "AV1 in MP4" \
...
demo codec no 13-codec-vp8.webm \
  "VP8 in WebM" \
...
demo codec yes 14-codec-mjpeg.mov \
...
demo codec no 15-codec-ffv1-lossless.mkv \
...
demo gop yes 16-gop-short-15.mp4 \
...
demo gop yes 17-gop-long-300.mp4 \
...
demo gop yes 18-gop-all-intra.mp4 \
...
demo gop yes 19-bframes-none.mp4 \
...
demo bitrate yes 21-bitrate-cbr-800k.mp4 \
...
demo bitrate yes 22-bitrate-starved-150k.mp4 \
...
demo track yes 23-track-rotated-90.mp4 \
  ...
  -- -display_rotation 90 -i "$OUT/01-reference.mp4" -c copy

demo track yes 24-track-video-only.mp4 \
...
demo track yes 25-track-vfr.mp4 \
  "Variable frame rate" \
...
demo track yes 26-resolution-half.mp4 \
  "Half resolution" \
...
demo metadata yes 27-metadata-rich.mp4 \
  "Rich metadata tags" \
...
NO_META=1 demo metadata yes 28-metadata-stripped.mp4 \
...
echo
echo "Wrote $((${#MANIFEST_ENTRIES[@]})) demo files and manifest.json to $OUT/:"
ls -l "$OUT" | tail -n +2 | awk '{printf "  %8.1f MB  %s\n", $5/1048576, $NF}'
