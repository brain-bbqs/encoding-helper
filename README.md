...
- **Three tabs** - **Inspect** (what the file is: metadata, the atom map, the bitrate and low-bandwidth playback tests, GOP/keyframe structure and the seeking test), **Reencode with FFmpeg** (build a command, try it on sampled seconds, run it over the whole file) and **Compare Quality** (sweep settings as a grid)
...
- **Low-bandwidth playback** - the card under the bitrate plot: whether the file would play through over a link of a given speed, from congested public wifi up to fast broadband or a rate you type in, and how long a viewer would have to wait before it starts. It is worked out from the sample table rather than measured, so it needs no server, no throttled connection and no decoding: a player is simulated downloading the file at that rate, waiting the startup spell you set, freezing when the frame it is due to show has not arrived, and refilling the same cushion before going on. It reports the slowest link that would play the file clean, the head start that would do the same at the link you picked, every freeze and how long it lasts, and plots the buffer over the whole of playback so a stream that sags toward empty is told apart from one that never does. A file whose `moov` index sits at the end is answered separately, since nothing plays at all until the last byte is down
...
3. Set a link speed in **Low-Bandwidth Playback**, under the bitrate plot, to see whether the file would stream over it and what a viewer would wait
4. Run the **Seeking Test**, at the bottom of that same tab, to measure nearest-keyframe distance and decode latency across the timeline (and see it plotted)
5. Tune CRF, preset, output resolution, keyframe interval, B-frames, faststart, and audio handling in the **Reencode with FFmpeg** tab
6. Run those settings on three seconds from the same tab — slide the track to the stretch worth looking at — and compare the result against the original side-by-side before committing to a full encode
7. Sweep several settings at once in the **Compare Quality** tab, then click the square you want and copy the `ffmpeg` command for it from the bottom of that page
8. Copy the generated `ffmpeg` command to run it natively, or click **Encode and Save** at the bottom of the same tab to transcode the whole video in the page
9. Click **Full Analysis** (to the right of the tabs) for the whole thing as one document: save it as HTML, print it to PDF, or copy it as Markdown
