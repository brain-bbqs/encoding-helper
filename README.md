...
4. Tune CRF, preset, output resolution, keyframe interval, B-frames, faststart, and audio handling in the **Reencode & CLI** tab
...
6. Try different CRF/preset values on a short clip in the **Compare Quality** tab and compare against the original side-by-side before running the full encode; raise **Segments** to sample several places in the file at once
7. Click **Full Analysis** (to the right of the tabs) for the whole thing as one document: save it as HTML, print it to PDF, or copy it as Markdown
