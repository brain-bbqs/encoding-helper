...
- **Downscale the output** - a **Resolution** dropdown (100/75/50/25%, labelled with the size each comes out at) in both the command builder and Compare Quality, scaling with `flags=lanczos` and `-2` for the height so the output keeps its aspect ratio and stays even-dimensioned. Resolution is the largest lever on file size and the one CRF cannot pull, so a downscaled encode is shown in the A/B window at the source's geometry, nearest-neighbour, rather than being judged through an interpolator
