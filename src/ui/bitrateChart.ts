...
export function renderBitrateChart(timeline: BitrateTimeline): SVGSVGElement {
  ...
  // Hover layer: a full-height band per window, so the pointer finds a window anywhere in the
  // column rather than only on the step itself, with the native tooltip carrying its numbers.
  for (const b of bins) {
