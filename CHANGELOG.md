...
- The document draws the **atom map** itself rather than only writing the box tree out as indented text. It is the same map the Atom Map tab shows, minus the parts that need a hand (no zoom, no breadcrumb, no hover readout), with its legend, and with each block's offset and size kept as a `title` so the exported HTML still reveals them on hover. Which labels fit is decided while the document is built, since nothing runs inside it to measure itself later. The text listing stays underneath, because a printed page has no hover to reveal a number with ([#17](https://github.com/brain-bbqs/encoding-helper/pull/17))
- Capped that listing at 400 boxes, with a line saying how many were left out. A fragmented recording has tens of thousands of boxes, which is a hundred pages of offsets in a document meant to be read; the map above it still draws every one ([#17](https://github.com/brain-bbqs/encoding-helper/pull/17))
