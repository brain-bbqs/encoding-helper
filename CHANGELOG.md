...
- Added a horizontal **Structure** view to the Atom Map, and made it the default. It is the box tree turned on its side: left to right across the file, each row down one level of nesting, so it stays a handful of rows tall whether the video runs ten seconds or ten hours. The indented tree grew one row per box, which for a fragmented recording (a `moof`+`mdat` pair per fragment) meant thousands of rows ([#14](https://github.com/brain-bbqs/encoding-helper/pull/14))
