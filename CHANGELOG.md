...
- Gave the Atom Map two horizontal views alongside the original indented tree, picked from a Structure/Bytes/Tree control that remembers the choice. Both run left to right with each row down one level of nesting, so they are a handful of rows tall whether the video runs ten seconds or ten hours. The tree grew one row per box, which for a fragmented recording (a `moof`+`mdat` pair per fragment) meant thousands of rows ([#14](https://github.com/brain-bbqs/encoding-helper/pull/14))
- **Structure**, the new default, gives every box room: siblings split their parent's width by how many boxes each subtree holds, so nothing is summarised away and the whole file is on screen at once. It is the tree turned on its side, and says nothing about size ([#14](https://github.com/brain-bbqs/encoding-helper/pull/14))
- **Bytes** measures the file itself, so a box is as wide as the bytes it occupies, with a byte-offset ruler above it. This is where a 99%-`mdat` file, or `moov` sitting before `mdat` for faststart, is visible at a glance. The trade is that a `moov` under 1% of the file has no room to open up, and is summarised until you zoom ([#14](https://github.com/brain-bbqs/encoding-helper/pull/14))
- Made both horizontal views zoomable: clicking any block narrows the view to it and a breadcrumb walks back out. Boxes too narrow to draw are never dropped, only merged into a block saying how many it stands for, and a merged run is cut as soon as it is wide enough to draw — so a 20,000-fragment file striates into blocks whose density is the fragmentation, rather than becoming one anonymous bar ([#14](https://github.com/brain-bbqs/encoding-helper/pull/14))
- Colored both views by which top-level box a byte belongs to (`moov`, `mdat`, `moof`, or everything else), with a legend and a readout naming the box under the cursor; a merged block takes the color of whatever fills most of it. The palette was checked for colorblind separation and contrast against both themes' card surface ([#14](https://github.com/brain-bbqs/encoding-helper/pull/14))
- Labelled `moof`, `mfra`, `free` and `skip` in the tree view, which previously tagged only `ftyp`, `moov` and `mdat` ([#14](https://github.com/brain-bbqs/encoding-helper/pull/14))
