...
- Each demo folds open to its description, its technical figures and the `ffmpeg` command that made it, with buttons to open it in the app or download it ([#28](https://github.com/brain-bbqs/encoding-helper/pull/28))
- The list can be filtered by name, group or extension, expanded or collapsed all at once, and narrowed to the files the app's MP4 parser can open ([#28](https://github.com/brain-bbqs/encoding-helper/pull/28))
